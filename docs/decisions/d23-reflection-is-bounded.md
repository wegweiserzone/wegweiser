...
Neither is on the scope fence, and neither had been ruled on. That is the gap this
records: not that they are missing, but that nobody had decided whether they should be.

## Decision

**The factor is bounded by construction and the rate is not policed.** RRL and DNS
Cookies come later.
...
Accepted knowingly: **a node on a public address can be used to reflect traffic at a
third party**, at up to 26.5× and only while the attacker has a large RRset to aim at. An
operator exposing this server to the internet should put a rate limiter in front of it. On Linux
...
What comes later, in this order:
...
2. **RRL** second, and only for cookieless clients. Rate limiting by traffic shape has false
   ...
   with knobs rather than a switch, and it is why it does not belong in a product whose
