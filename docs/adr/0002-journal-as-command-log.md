# ADR 0002 — The journal is the replication log
...
Raft is out of v0.1 scope. The decision is not out of scope, because it constrains the shape
of the write path that v0.1 builds.
...
- Journal truncation becomes a cluster concern later: a lagging follower needs the entries a
  ...
  mechanism for that case, which is why they exist in v0.1 even though single-node rollback
  could manage without them.
- v0.1 pays a small cost: the write path is split into command / validate / expand / apply
