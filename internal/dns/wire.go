...
type Responder struct {
	...
	// keys is where the server publishes its TSIG keys, or nil for a responder
	// nobody wired one into. A pointer to the holder rather than a copy of the
	// ring, so that a key created while this responder is alive is seen by the
...
func (r *Responder) Respond(snap *Snapshot, query []byte, tr Transport, out []byte) ([]byte, error) {
	...
	// A signed query is answered signed, whatever the answer turns out to be
	// (RFC 8945 §5.3). This is not a transfer-only concern: BIND signs the SOA
	// refresh it sends before asking for a zone, and discards an unsigned
	// answer to it, so a secondary configured with a key never gets as far as
	// the transfer.
	// Only a message that parsed: whatever survived a failed unpack is not
	// something to verify a MAC against, and the table below has an answer for
	// it already.
	if unpackErr == nil {
		if refusal := r.checkTSIG(query); refusal != nil {
			return r.refuseTSIG(refusal, out)
		}
	}
	...
	case r.req.Opcode != wire.OpcodeQuery:
		// NOTIFY is addressed to a secondary and UPDATE to a server that takes
		// dynamic updates. Nothing transfers into this one, so it is never
		// either, and answering would be a claim (docs/decisions/ D27).
...
// checkTSIG verifies a signature the query carries, and reports the refusal
// RFC 8945 §5.2 asks for where it does not hold.
//
// A query carrying none is the ordinary case and costs a nil check.
func (r *Responder) checkTSIG(query []byte) *tsigError {
	if r.keys == nil {
		return nil
	}
	holder := r.keys.Load()
	if holder == nil || len(holder.ring) == 0 {
		return nil
	}

	signed, err := verifyTSIG(holder.ring, query, &r.req)
...
// refuseTSIG answers a query whose signature did not hold.
//
// Nothing is resolved: a request this server cannot attribute gets the refusal
// and no data, whatever it asked for.
func (r *Responder) refuseTSIG(refusal *tsigError, out []byte) ([]byte, error) {
	r.resp.Rcode = wire.RcodeNotAuth

	packed, err := signRefusal(&r.resp, refusal.rr, refusal.key, refusal.code, time.Now())
	if err != nil {
		return nil, err
	}
	r.ev.Rcode = r.resp.Rcode
	r.ev.Dropped = false
	return append(out[:0], packed...), nil
}
...
func (r *Responder) answer(snap *Snapshot) {
	...
	//
	// Measured and left alone (docs/decisions/ D12). The kernel is about 95%
...
func (r *Responder) finish(reqOPT *wire.OPT, tr Transport, out []byte) ([]byte, error) {
	...
	if r.signer != nil {
		// A signature is not itself something to truncate, so the room it needs
		// comes off the budget before the sections are cut to fit.
		budget -= tsigReserve
	}
	r.resp.Truncate(budget)

	if r.signer != nil {
		signed, serr := r.signer.sign(&r.resp)
		if serr != nil {
			return nil, serr
		}
		r.ev.Rcode = r.resp.Rcode
		...
		r.ev.Dropped = false
		return append(out[:0], signed...), nil
	}
