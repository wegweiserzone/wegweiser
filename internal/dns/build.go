...
// the presentation format that ADR 0001 makes canonical: the text came out of
...
func soaRR(z *zone.Zone, ttl zone.TTL) (wire.RR, error) {
	...
	b.WriteString(z.Name.String())
	b.WriteByte('\t')
	b.WriteString(ttl.String())
	b.WriteString("\tIN\tSOA\t")
	b.WriteString(z.SOA.RData())

	rr, err := wire.NewRR(b.String())
	if err != nil {
		return nil, fmt.Errorf("SOA of %q (%s): %w", z.Name, b.String(), err)
	}
	return rr, nil
}
