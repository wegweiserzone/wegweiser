...
func (a *Applier) Reconcile(ctx context.Context, zid zone.ZoneID, meta Meta) (*Result, error) {
	...
	b, res, err := a.PlanReconcile(ctx, zid, meta)
	if err != nil {
		return nil, err
	}
...
// PlanReconcile works out what filling a zone would amount to, and leaves the
// store as it found it. See [Applier.Plan] for what a plan is and is not.
func (a *Applier) PlanReconcile(
	...
	b, err := a.planning(ctx, func(tx store.Tx) (*Batch, error) {
		z, zerr := tx.ZoneByID(ctx, zid)
		if zerr != nil {
			return nil, zerr
		}

		cs := &changeSet{}
		if rerr := a.reconcileInto(ctx, tx, cs, z, res); rerr != nil {
			return nil, rerr
		}
		if cs.empty() {
			return &Batch{}, nil
