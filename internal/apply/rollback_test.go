...
func TestRollbackAcrossCommitsRecordedInOneMillisecond(t *testing.T) {
	t.Parallel()
	f := newFixture(t)

	// The fixture's clock does not move, so the journal cannot tell these apart
	// by the time they were recorded and the listing falls back to ordering by
	// identifier, which is no order at all. What says which one followed which
	// is the chain of serials (D2).
	target := f.serial()
	want := f.records()
