...
func TestRecordLifecycle(t *testing.T) {
	...
	t.Run("no reverse zone covers it, and that is said rather than swallowed", func(t *testing.T) {
		// docs/decisions/ D6: the hint is structured data, not prose.
...
// TestAuth covers docs/decisions/ D5 from the outside.
...
// TestReverseConflict covers docs/decisions/ D3 through the API: a second
...
// TestSession covers the browser half of docs/decisions/ D5.
...
func TestImportAndExport(t *testing.T) {
	...
	t.Run("the serial it arrived with is the serial it starts at", func(t *testing.T) {
		// docs/decisions/ D2: restarting at 1 makes a migrated zone look
...
// TestSettings covers the server-wide settings of docs/decisions/ D3 and
...
// recordingTransfers is a data plane that remembers what it was last told.
type recordingTransfers struct {
	mu   sync.Mutex
	last dns.Transfers
}

func (r *recordingTransfers) SetTransfers(t dns.Transfers) {
	r.mu.Lock()
	defer r.mu.Unlock()
	r.last = t
}

func (r *recordingTransfers) mayTransfer(client string) bool {
	r.mu.Lock()
	defer r.mu.Unlock()
	if r.last == nil {
		return false
	}
	return r.last.MayTransfer(
		netip.MustParseAddr(client), zone.Name{}, zone.MustParseName("example.com."))
}

// TestTransferListReachesTheQueryPath is the half that matters for D26:
// the list is stored, and the server that enforces it is told, so a change
// takes effect without a restart.
func TestTransferListReachesTheQueryPath(t *testing.T) {
	t.Parallel()
	...
	if len(got.TransferAllow) != 0 {
		t.Errorf("a fresh server transfers to %v, want nobody", got.TransferAllow)
	}
	if sink.mayTransfer("192.0.2.7") {
		t.Error("the query path allowed a transfer before anything was set")
	}

	var after gen.Settings
	h.decode(h.do(http.MethodPatch, "/settings", gen.UpdateSettings{
		TransferAllow: &[]string{"192.0.2.0/24", "2001:db8::1"},
	}), http.StatusOK, &after)
	if want := []string{"192.0.2.0/24", "2001:db8::1/128"}; !slices.Equal(after.TransferAllow, want) {
		t.Errorf("the list came back as %v, want %v", after.TransferAllow, want)
	}
	if !sink.mayTransfer("192.0.2.7") {
		t.Error("the query path was not told about the change")
	}
	if sink.mayTransfer("198.51.100.7") {
		t.Error("the query path allows a client outside the list")
	}
	...
	t.Run("something that is not an address is refused", func(t *testing.T) {
		resp := h.do(http.MethodPatch, "/settings",
			map[string]any{"transferAllow": []string{"ns1.example.com."}})
		if resp.StatusCode != http.StatusUnprocessableEntity {
			t.Errorf("status = %d, want 422", resp.StatusCode)
		}
	})

	t.Run("the list can be taken away again", func(t *testing.T) {
		var empty gen.Settings
		h.decode(h.do(http.MethodPatch, "/settings", gen.UpdateSettings{
			TransferAllow: &[]string{},
		}), http.StatusOK, &empty)
		if len(empty.TransferAllow) != 0 {
			t.Errorf("the list is %v, want it emptied", empty.TransferAllow)
		}
		if sink.mayTransfer("192.0.2.7") {
			t.Error("the query path still allows a transfer")
		}
	})
}
